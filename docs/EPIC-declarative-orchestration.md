...
format, the monitor port, `/healthz` — lives in a manifest under
`examples/nats/`. The control plane holds no NATS knowledge.

> **Revision note.** The first draft made `AssignmentSet` a first-class object
> with its own proto, table, RPCs and controller. Reviewing the implementation
> showed why that was wrong: of ~1366 lines carrying the NATS name, roughly 14
> were actually about NATS. The rolling, reservation, deadline and status
> machinery is entirely workload-agnostic. The epic's own rule — "no
> `if strategy == \"nats\"` in the reconciler" — had been applied to the agent
> and quietly broken in the control plane. Kubernetes keeps `EtcdCluster` in an
> Operator CRD rather than the core API for the same reason.
...
3. A `AssignmentSet` object is stored independently of assignments. A CP
...
| Field | Job |
|-------|-----|
| `kind` | Dispatch. `StrategyAssignment` → assign API; `AssignmentSet` → cluster store. Never sent to the agent. |
| `metadata.name` | Stable key. Cluster `trading` is `assignment_sets.name`. |
...
flowchart TB
  ...
  API --> ClusterStore["AssignmentSet store"]
  ClusterStore --> Ctrl["AssignmentSet controller"]
...
2. **Cluster loop (new):** one `AssignmentSet` spec + fleet `StrategyView`.
...
`ApplyAssignmentSet` **only persists the cluster object**. If it also
...
unchanged). The AssignmentSet controller passes `false`, because E3 makes it
...
E1 assign service + store-level no-op + shared view/Hub seams
     ...
     ├─────────────────────────┐
     ▼                         ▼
E3 AssignmentSet store      E2 ApplyAssignment RPC
     │                         │
     ▼                         │
E4 AssignmentSet controller      │
     │                         │
     ▼                         │
...
### E3 — `AssignmentSet` resource (persist only)

**Description**

Introduce `kind: AssignmentSet` as its own stored object. Human Apply
...
- Proto module (e.g. `proto/strategyplatform/v1/assignmentset.proto`) + RPCs:
  `ApplyAssignmentSet`, `GetAssignmentSet`, `ListAssignmentSets`,
  `DeleteAssignmentSet` (delete may be “mark deleting”; actual Undeploy
  is E4).
- Store: Postgres migration (`0010_assignment_sets.sql`) + memory
...
- [ ] `apply -f` on the example `AssignmentSet` hits `ApplyAssignmentSet`,
...
Read-only (or apply-via-form) view of `AssignmentSet` list and per-server
phase, reusing `WatchMachine` / GetAssignmentSet. Must not become a second
...
- Writes, if any, only `ApplyAssignmentSet` / delete.
...
3. `E3: Persist AssignmentSet objects and reserve their strategy name`
4. `E4: Reconcile AssignmentSet into rolling per-machine assignments`
...
7. `E7: Read-only AssignmentSet UI`
...
| Controller and UI both write assignments | Only the controller writes member assignments; ApplyAssignmentSet is spec-only |
