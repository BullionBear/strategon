...
Named credentials (`secret.<name>` in `env` / `member.vars`) are specified in
[DESIGN-sealed-env.md](DESIGN-sealed-env.md) (SecretManagement).
...
template fails. Env values that start with `secret.` are SecretManagement
references (`secret.<name>`); see [DESIGN-sealed-env.md](DESIGN-sealed-env.md).
Resolution is keyed off the **launch artifact**, not
