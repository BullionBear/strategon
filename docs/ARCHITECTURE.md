...
so they cannot reach a process as literal text. Resolution is keyed off the
**launch artifact**, not `spec.driver`, so auto-rollback from OCI to a
previous BINARY still expands the same mount list.
